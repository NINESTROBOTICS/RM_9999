...
public class TESTROBOT {
    ...
    double rangesumB;
    double rangesumR;
    double Kp = 0.0008; /*increase the Kp to enhance the oscillation and reduce the adjusting time */
    double Ki = 90000; /*increase the Ki to decrease the oscillation and a longer adjusting time */
    double Kp1 = 0.003;
    double Ki1 = 150000;

    public TESTROBOT() {/* Constructor */}
    ...
    public void initializeRobot(OpMode OP_MODE, String TEAM_COLOR) {
       ...
       // motorRISE.setMode(DcMotor.RunMode.RUN_TO_POSITION);
        ...
        sensorColor2 = myOpMode.hardwareMap.colorSensor.get("color2");

        gyro = myOpMode.hardwareMap.get(ModernRoboticsI2cGyro.class, "gyro");
        gyro.calibrate();

        //rangeSensor = myOpMode.hardwareMap.get(ModernRoboticsI2cRangeSensor.class, "sensor_range");
        ...
        sensorDistance2 = myOpMode.hardwareMap.get(DistanceSensor.class, "sensor_distance");
        ...
        servoJ.setPosition(0.7);

        servoS1.setPosition(0.657);      //0.01 = 1.8℃//64  //64
        servoS2.setPosition(0.311);  //35                   //308

        //servoCR.setPosition(0.1);
        servoW.setPosition(Wposition);
        servoD.setPosition(1);
        //servoCL.setPosition(0.1);
    ...
    public void addRobotTelemetry() {
        ...
        //myOpMode.telemetry.addData("Direction",gyro.getIntegratedZValue());
    ...
    public void adjustWithEncoder() {
        ...
        setMotorPower(power[L1] / 1.3, power[R1] / 1.3, power[L2] / 1.3, power[R2] / 1.3);//2.1
    }
    ...
    public void runDistanceHALFSPEED(double xDistance, double yDistance, double speed) {
        ...
        while (!hasArrived() && IsRunning == true) adjustWithEncoderHALF();
    ...
    public void rotateToTargetDirection(int targetDirection) {
        /**
         * 基于机器人初始化时的朝向设定目标位置
     ...
     * //// public double getDistanceCM2()
     * {
     * return rangeSensor2.getDistance(DistanceUnit.CM);
     * }
     * public double getDistanceCM3() { return rangeSensor3.getDistance(DistanceUnit.CM); }
     * <p>
     * public void standOffWall(double distance)
     * {
     * while (Math.abs(getDistanceCM() - distance) > 3)
     * {
     * //if( getDistanceMM() == Math.NaN )
     * double power_ = (getDistanceCM()*10 - distance*10) * ENCODER_PROPORTION * 3;
     * <p>
     * if (Math.abs(power_) < 0.10)
     * {
     * if (power_ != 0)
     * {
     * power_ = power_ / Math.abs(power_) * 0.10;
     * }
     * }
     * <p>
     * power_ = Range.clip(power_, -0.30, 0.30);
     * <p>
     * mecanumRun(power_, 0, 0);
     * }
     * <p>
     * }
     * public void standOffWall3(double distance) {
     * while (Math.abs(getDistanceCM3() - distance) > 2.5)
     * {
     * //if( getDistanceMM2() == Math.NaN )
     * double power_ = (getDistanceCM3() * 10 - distance * 10) * ENCODER_PROPORTION * 3;
     * <p>
     * if (Math.abs(power_) < 0.10) {
     * if (power_ != 0) {
     * power_ = power_ / Math.abs(power_) * 0.10;
     ...
     * power_ = Range.clip(power_, -0.30, 0.30);
     * <p>
     * mecanumRun(0, -power_, 0);
     * }
     * }
     * public void standOffWallR(double distance)
     * {
     * /*
     * while (rangeSensor.getDistance(DistanceUnit.CM)<=distance-1 || rangeSensor.getDistance(DistanceUnit.CM)>=distance+1)
     * {
     * if (rangeSensor.getDistance(DistanceUnit.CM) < distance-1)
     * {
     * while (rangeSensor.getDistance(DistanceUnit.CM) < distance-1)
     * {
     * double y = rangeSensor.getDistance(DistanceUnit.CM);
     * double error = distance - y;
     * rangesum = rangesum + error;
     * double power = (Math.abs(rangeSensor.getDistance(DistanceUnit.CM) *Kp1)) + (rangesum/Ki1) ;
     * mecanumRun(-power, 0, 0);
     * }
     * }
     * else if (rangeSensor.getDistance(DistanceUnit.CM) > distance+1)
     * {
     * while (rangeSensor.getDistance(DistanceUnit.CM) > distance+1)
     * <p>
     * {
     ...
     * }
     * <p>
     * while (Math.abs(getDistanceCM() - distance) > 2)
     * {
     * //if( getDistanceMM() == Math.NaN )
     * double power_ = (getDistanceCM()*10 - distance*10) * ENCODER_PROPORTION * 3;
     * <p>
     ...
     * }
     * <p>
     * power_ = Range.clip(power_, -0.25, 0.25);
     * <p>
     * mecanumRun(power_, 0, 0);
     * }
     * <p>
     ...
     * }
     * <p>
     * public void rangebyLXHB(double distance)  //右侧range1
     * {
     * while (rangeSensor.getDistance(DistanceUnit.CM)<=distance-1 || rangeSensor.getDistance(DistanceUnit.CM)>=distance+1)
     * {
     * if (rangeSensor.getDistance(DistanceUnit.CM) < distance-1)
     ...
     * double y = rangeSensor.getDistance(DistanceUnit.CM);
     * double error = distance - y;
     * rangesumB = rangesumB + error;
     * double power = (Math.abs(rangeSensor.getDistance(DistanceUnit.CM) *Kp1)) + (rangesumB/Ki1) ;
     * mecanumRun(-power, 0, 0);
     * }
     * }
     * else if (rangeSensor.getDistance(DistanceUnit.CM) > distance+1)
     ...
     * }
     * }
     * public void rangebyLXHR(double distance)    //左侧range2
     * {
     * while (rangeSensor2.getDistance(DistanceUnit.CM) <= distance - 1 || rangeSensor2.getDistance(DistanceUnit.CM) >= distance + 1) {
     * if (rangeSensor2.getDistance(DistanceUnit.CM) < distance - 1) {
     * while (rangeSensor2.getDistance(DistanceUnit.CM) < distance - 1) {
     * double y = rangeSensor2.getDistance(DistanceUnit.CM);
     * double error = distance - y;
     * rangesumR = rangesumR + error;
     * double power = (Math.abs(rangeSensor2.getDistance(DistanceUnit.CM) * Kp1)) + (rangesumR / Ki1);
     * mecanumRun(power, 0, 0);
     * }
     * } else if (rangeSensor2.getDistance(DistanceUnit.CM) > distance + 1) {
     * <p>
     * }
     * {
     * while (rangeSensor2.getDistance(DistanceUnit.CM) > distance + 1)
     ...
     * }
     * }
     * public void standOffWallB2(double distance)
     * {/*
     * while (rangeSensor2.getDistance(DistanceUnit.CM)<=distance-1 || rangeSensor2.getDistance(DistanceUnit.CM)>=distance+1)
     * {
     * if (rangeSensor2.getDistance(DistanceUnit.CM) < distance-1)
     ...
     * double error = distance - y;
     * rangesum = rangesum + error;
     * double power = (Math.abs(rangeSensor2.getDistance(DistanceUnit.CM) *Kp1)) + (rangesum/Ki1) ;
     * mecanumRun(power, 0, 0);
     * }
     * }
     * else if (rangeSensor2.getDistance(DistanceUnit.CM) > distance+1)
     ...
     * }
     * <p>
     * while (Math.abs(getDistanceCM2() - distance) > 3)
     * {
     * //if( getDistanceMM() == Math.NaN )
     * double power_ = (getDistanceCM2()*10 - distance*10) * ENCODER_PROPORTION * 3;
     * <p>
     * if (Math.abs(power_) < 0.10)
     * {
     * if (power_ != 0)
     ...
     * {
     * if (teamColor==true) OriginalDirection=0; //RED
     * else  OriginalDirection=180;//BLUE
     * }
     */
    ...
    public void Jewelry() {
        ...
        if (colorDetected() == teamColor) //ball detected
        {
            ...
            servoW.setPosition(Wposition + 0.35);
    ...
    public void Distancejudge()//正式使用前请重新关联新传感器和名字和调平移方向
    {
        ...
        {
            if (sensorDistance2.getDistance(DistanceUnit.MM) <= 20) {
                while (sensorDistance2.getDistance(DistanceUnit.MM) <= 20 + EPSdistance && sensorDistance2.getDistance(DistanceUnit.MM) >= 20 - EPSdistance) {
                    v = sensorDistance2.getDistance(DistanceUnit.MM);
                    deltav = 20 - v;
                    power = deltav / 500;
                    mecanumRun(power, 0, 0);
                }
            } else if (sensorDistance2.getDistance(DistanceUnit.MM) >= 20) {
                while (sensorDistance2.getDistance(DistanceUnit.MM) >= 20 + EPSdistance && sensorDistance2.getDistance(DistanceUnit.MM) <= 20 - EPSdistance) {
                    v = sensorDistance2.getDistance(DistanceUnit.MM);
                    deltav = v - 20;
                    power = deltav / 500;
    ...
    void Navigation() {
        if (sensorDistance2.getDistance(DistanceUnit.CM) <= 8.6) {            //12.65
            while (sensorDistance2.getDistance(DistanceUnit.CM) <= 8.6 && IsRunning == true) {
                double v = sensorDistance2.getDistance(DistanceUnit.CM);
                double deltav = 8.6 - v;
        ...
        } else if (sensorDistance2.getDistance(DistanceUnit.CM) >= 8.6) {
            while (sensorDistance2.getDistance(DistanceUnit.CM) >= 8.6 && IsRunning == true) {
                double v = sensorDistance2.getDistance(DistanceUnit.CM);
                double deltav = v - 8.6;
                double power = deltav / 55;
    ...
    public void DistanceSWITCH() {
        ...
        if (sensorDistance2.getDistance(DistanceUnit.MM) <= 20 + EPSdistance && sensorDistance2.getDistance(DistanceUnit.MM) > 20 - EPSdistance) {
            stopAllMotors();
        } else {
            Distancejudge();
        }
    ...
    void GlyphServo() {
        servoS1.setPosition(0.02);
        servoS2.setPosition(0.948);  //980
    }

    void GlyphServoPin() {
        servoS1.setPosition(0.57);
        servoS2.setPosition(0.398);
    }

    void GlyphServoDown() {
        servoS1.setPosition(0.657);      //0.01 = 1.8℃
        servoS2.setPosition(0.311);
    }
    ...
    void SOCL2() {
        ...
        do {
            mecanumRun(0, -0.14, 0);
            if (sensorDistance2.getDistance(DistanceUnit.CM) <= 30) {
                open = false;
            } else {
                open = true;
            }
    ...
    public void telewithdistance() {
        if (sensorDistance2.getDistance(DistanceUnit.CM) <= 8.6)
        {            //12.65
            if (sensorDistance2.getDistance(DistanceUnit.CM) <= 8.6 && IsRunning == true)
            {
                double v = sensorDistance2.getDistance(DistanceUnit.CM);
                double deltav = 8.6 - v;
                double power = deltav / 55;
